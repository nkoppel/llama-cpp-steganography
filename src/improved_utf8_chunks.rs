...
pub fn utf8_chunks(this: &[u8]) -> Utf8Chunks<'_> {
    Utf8Chunks { source: this }
}
// }
...
impl<'a> Utf8Chunk<'a> {
    ...
    /// Returns true if [`invalid`] contains bytes and the invalid sequence occurred due to an
    /// unexpected end of input.
    ///
    /// [`invalid`]: Self::invalid
    #[must_use]
    pub fn unexpected_end(&self) -> bool {
        self.unexpected_end
    }
}
    ...
    fn next(&mut self) -> Option<Utf8Chunk<'a>> {
        ...
        match from_utf8(self.source) {
            ...
            Err(e) => {
                let chunk_len = e
                    .error_len()
                    .map(|len| e.valid_up_to() + len)
                    .unwrap_or(self.source.len());

                let chunk;
                (chunk, self.source) = self.source.split_at(chunk_len);
                let (valid, invalid) = chunk.split_at(e.valid_up_to());

                Some(Utf8Chunk {
                    valid: unsafe { from_utf8_unchecked(valid) },
                    invalid,
                    unexpected_end: e.error_len().is_none(),
                })
            }
        }
    }
